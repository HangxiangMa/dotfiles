-- ==============================================================================
-- Neovim Lua syntax additions for C++
...
--
-- Notes:
-- - Vim's :syntax items do not have a dedicated Lua API; we use vim.cmd inside
--   the buffer context. Highlight links use vim.api.nvim_set_hl().
-- - Global toggles honored:
--     g.cpp_attributes_highlight  (default: 0)
--     g.cpp_simple_highlight      (default: 0)
--
-- Install:
--   require("syntax.cpp").setup()
-- Or in after/syntax/cpp.lua:
--   require("syntax.cpp").apply(0)
...
-- Execute :syntax command in buffer context
...
function M.apply(bufnr)
	...
	-- C++ attributes
	---------------------------------------------------------------------------
	if get_g('cpp_attributes_highlight', 0) ~= 0 then
		-- Use [=[ ... ]=] so embedded ']]' doesn't terminate the Lua string
		syn_in_buf(bufnr,
			[=[syntax region cppAttribute matchgroup=cppAttributeBrackets start='\[\[' end=']]' contains=cString]=])
		vim.api.nvim_set_hl(0, 'cppAttribute', { link = 'Macro' })
		vim.api.nvim_set_hl(0, 'cppAttributeBrackets', { link = 'Identifier' })
	end
	...
	if vim.fn.exists('cpp_no_cpp23') == 0 then
		...
		syn_in_buf(bufnr, [[syntax keyword cppSTLtypedef stacktrace dextents]])
		syn_in_buf(bufnr,
		...
		syn_in_buf(bufnr, [[syntax keyword cppSTLfunction invoke_r]])
		syn_in_buf(bufnr, [[syntax keyword cppSTLtype expected unexpected unexpect_t bad_expected_access]])
		syn_in_buf(bufnr, [[syntax keyword cppSTLvariable unexpect]])
	end
	...
	-- Standard C++ modifiers (inline, virtual, etc.) are Statements, not types
	vim.api.nvim_set_hl(0, 'cppModifier', { link = 'Statement' })

	-- Simple highlight option: link common C++ groups to Statement
	if get_g('cpp_simple_highlight', 0) ~= 0 then
		vim.api.nvim_set_hl(0, 'cppStructure', { link = 'Statement' })
		vim.api.nvim_set_hl(0, 'cppExceptions', { link = 'Statement' })
		vim.api.nvim_set_hl(0, 'cppStorageClass', { link = 'Statement' })
	end
end

function M.setup()
	local group = vim.api.nvim_create_augroup('CPP_Lua_Syntax', { clear = true })
