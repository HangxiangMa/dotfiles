...
function H.validate_and_merge_config(user_config)
	...
	if config.opacity < 0 or config.opacity > 1 then
		vim.notify("InactiveRegions: opacity must be between 0.0 and 1.0, got " .. config.opacity, vim.log.levels.WARN)
		config.opacity = 0.5
	end
...
function H.regions_changed(filename, new_regions)
	...
	for i, region in ipairs(new_regions) do
		...
		if
			not cached_region
			or region.start.line ~= cached_region.start.line
			or region.start.character ~= cached_region.start.character
			or region["end"].line ~= cached_region["end"].line
			or region["end"].character ~= cached_region["end"].character
		then
			return true
		end
	end
...
function H.process_regions_coroutine(bufnr, filename, regions, start_time, render_top, render_bot)
	...
	H.log(
		"Completed processing %d regions with %d highlights in %.2fms",
		#regions,
		total_highlights,
		processing_time_ms
	)
...
function H.resolve_highlight_color(group_name, attr)
	...
	local function resolve(name)
		...
		if val then
			return type(val) == "string" and val:match("^#") and val or string.format("#%06x", val)
		end
...
function H.setup_debug_commands()
	...
	vim.api.nvim_create_user_command("InactiveRegionsDebugWord", function()
		...
		print(
			string.format(
				"Immediate char highlight enabled: %s",
				tostring(InactiveRegions.config.immediate_char_highlight)
			)
		)
