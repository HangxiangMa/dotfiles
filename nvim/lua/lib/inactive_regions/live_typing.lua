...
local function update_word_highlight_with_treesitter(bufnr, row, start_col, end_col, word)
  ...
  local all_extmarks = vim.api.nvim_buf_get_extmarks(
    bufnr, D.ns, { row, 0 }, { row, -1 }, { details = true }
  )
  ...
  vim.api.nvim_buf_add_highlight(
    bufnr, D.ns, highlight_group, row, start_col, end_col + 1
  )
...
local function apply_temp_highlights_to_range(bufnr, start_row, start_col, end_row, end_col)
  ...
  for line = start_row, end_row do
    ...
    local existing_marks = vim.api.nvim_buf_get_extmarks(
      bufnr, D.ns, { line, line_start_col },
    ...
    local hl_id = vim.api.nvim_buf_add_highlight(
      bufnr, D.ns, temp_group, line, line_start_col, line_end_col
    )
    ...
    D.log("Applied temp highlight to line %d, cols %d-%s",
...
local function apply_live_typing_highlights(bufnr)
  ...
  if D.config.immediate_char_highlight and start_col < end_col then
    ...
    D.log("Applied live typing highlights to entire word '%s' at buffer %d, range (%d,%d-%d)",
...
local function reapply_typed_highlights(bufnr)
  local filename = vim.api.nvim_buf_get_name(bufnr)
  if not filename or filename == "" then return end
  if not is_cursor_in_inactive_region(bufnr) then return end
  ...
  if #captures > 0 then
    for _, capture in ipairs(captures) do
      if capture.capture then
        ...
        vim.api.nvim_buf_add_highlight(
          bufnr, D.ns, highlight_group, row,
          math.max(0, col - 1), col + 1
        )
      end
    end
...
function M.setup_autocmds(augroup)
  vim.api.nvim_create_autocmd("InsertEnter", {
    group = augroup,
    callback = function(args) update_cursor_inactive_status(args.buf) end,
  })
  ...
  vim.api.nvim_create_autocmd("InsertLeave", {
    group = augroup,
    callback = function(args) clear_temp_highlights(args.buf) end,
  })
